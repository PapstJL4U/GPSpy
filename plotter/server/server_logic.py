...
def load_all_tiles(
    path_to_gps_file: str, tile_list: list[tuple], zoom: int = 15
) -> None:
    """load all tiles"""
    for i, tile in enumerate(tile_list):
        ...
        mao.save_png(
            pathy.joinpath(f"Tile_{i}_{zoom}_{north}_{west}_{south}_{east}.png")
        )


def plot_my_path(
    file_path: str = "None", only_location: tuple = None, df: pd.DataFrame = None
) -> str:
...
def plot_my_mapbuilder(
    path_to_tiles_folder: str, only_location: tuple = None, df: pd.DataFrame = None
) -> str:
    """do all the loading, plotting and saving files"""
